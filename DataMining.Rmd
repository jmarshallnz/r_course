...
## What is Data Mining?{.flexbox .vcenter}
...
```{r}
todo <- "swiss example 1"
```

```{r}
todo <- "swiss example 2"
```

```{r}
todo <- "swiss example 3 with line on it"
```
...
```{r}
todo <- "Hopefully intersting LDA example..."
```

- Produce predictions of each class for the `.csv` test set using the `predict` command.
...
```{r}
TODO <- "A heirarchical clustering example"
```
